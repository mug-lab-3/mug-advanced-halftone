...
`MugAdvancedHalftone.fuse` では、ドットの配置を規則正しく均等に並べるだけでなく、アナログ印刷の荒れやノイズを再現するために、ドットの「位置」や「サイズ」「縦横比」「角度」を意図的にランダムにズラす**ジッター（Jitter）機能**が実装されています。
...
### 4-4. 角度のズレ（Angle Jitter）の計算
さらに別の独立した乱数（`randAngle`）を用いて、ドットがランダムな方向に回転するための変動を与えます。
```c
