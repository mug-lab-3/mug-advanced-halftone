# MugAdvancedHalftone.fuse 技術仕様書

## 1. 概要 (Overview)

`MugAdvancedHalftone.fuse` は、DaVinci Resolve/Fusion 向けの高品質なリアルタイム・ハーフトーン生成プラグイン (v3.0) です。
六角形グリッドまたは直交グリッドに基づくドットパターンを用いて、印刷物のような視覚的特徴（網点表現）をシミュレートします。
...
### 主な特徴
- **六角形 / 直交ドットグリッド**: 自然なハニカム配置と、整列したスタック配置の切り替えが可能。
- **ドット形状バリエーション**: 円形（Circle）、四角形（Square）、ひし形（Diamond）、線（Line）、網線（Crosshatch）から選択可能。
- **アナログ印刷のシミュレート（Jitter）**: 擬似乱数を用いた位置ズレ（Position Jitter）、サイズムラ（Size Jitter）、縦横比のブレ（Aspect Jitter）、回転のブレ（Angle Jitter）による有機的でアナログな不完全さの再現。滑らかなアニメーションに対応する連動ノイズ（Jitter Noise Phase）を実装。
- **柔軟なトーン制御**: 輝度反転、ドットゲイン補正、非線形なドットサイズカーブによる緻密なコントラスト設計。
- **高品質なエッジ処理**: セル距離に基づく計算とソフトネス調整が可能なアンチエイリアス処理。
- **ライン・ハッチング・モード**: ドットの代わりに「線」で画像を表現します。通常のハッチングに加え、線が交差する「クロスハッチング」もサポート。
- **カラー表現と色収差 (RGB Shift)**: ユーザー定義色や入力画像の直接合成に加え、中心からの距離に比例してRGBチャンネルを放射状にズラしてサンプリングし、レンズ固有の色収差をシミュレート。
...
graph TD
    ...
    C_K --> B3["cell_info3<br/>拡張情報 (Extra)"]
...
| **cell_info3** | `.x` | `finalAngle` | Jitterを加味して算出されたこのセル専用の回転角度 (度) |
| | `.y` | `cosA` | 回転行列用の $\cos$ 定数 (事前計算済み) |
| | `.z` | `sinA` | 回転行列用の $\sin$ 定数 (事前計算済み) |
| | `.w` | `recipAspect` | アスペクト比の逆数 $1/Aspect$ (事前計算済み) |
...
### ステージ 1: セルグリッド設計 (Screen Density / Staggered Grid)
UI からの `Screen Density` を基に、グリッド構成のピッチ（間隔）をカーネル内で算出します。

1. **六角形（スタガー）配置**: `Staggered Grid` がオンの場合、Y方向のピッチは $X \times \frac{\sqrt{3}}{2}$ となります（コード内では定数 `SIN60 = 0.866025` を乗算）。奇数行 (`isOddRow`) は常にX方向に半セル分オフセットされ、蜂の巣状（ハニカム）の配置を実現します。
2. **直交（ストレート）配置**: `Staggered Grid` がオフの場合（Circle, Square, Diamondのみ）、Y方向のピッチは $X$ と同等になり、グリッドは完全に整列します。

*Line / Crosshatch モードでは、内部的なサンプリング効率のため常にスタガー配置のグリッドが使用されます。*
...
1. **Min Dot Radius**: 算出された仮の半径が `MinDotRadius` 未満であれば、ノイズまたは極端なハイライト（あるいは反転時はシャドウ）とみなして $R_{dot}=0$ にします。これにより、技術的な最小サイズ制限と、芸術的なトーンの切り捨てを一つのパラメーターで同時に制御します。
2. **最大限界値 (Clip Dot Radius)**: 極端なインク溜まりを防ぐため、物理的な最大セル半径 (`maxCellRadius`) に上限比率 (`ClipDotRadius`) を掛けた値でクランプします。
...
- **Line / Crosshatch (線)**: 
  画像全体の座標系に基づいたグローバルな平行線パターンを用いて距離を計算します。
  $d = |(P \cdot N) \pmod{\text{pitch}} - (\text{pitch}/2)|$
  これにより、セル境界を越えて完全に連続したラインが形成されます。
  
  **Crosshatch (網線)** では、2方向目のライン（セカンダリ・ハッチ）がシャドウ部（暗い領域）にのみ現れる「プロフェッショナル・エングレービング・ロジック」を採用しており、伝統的な銅版画のような密度表現を実現しています。
...
| | Staggered Grid | On | Checkbox | ドットをレンガ状に互い違いに配置します。 (Circle, Square, Diamond 選択時) |
...
| | Min Dot Radius | 0.05 | 0.0 - 1.0 | 描画する最小のドット半径。これ未満のゴミのようなドットや、明るすぎるハイライトを排除します。 |
...
| | Angle Jitter | 0.0 | 0.0 - 1.0 | 各ドットの回転角度を個別にランダムにブレさせる強さ（最大 ±90度）。 |
| **Dot Color** | RGB Shift | 0.0 | 0.0 - 0.1 | 画像中心からの距離に比例してRGBチャンネルをズラしてサンプリングし、放射状の色収差を再現。（割合指定）|
...
| **Background Color** | Blend With Input | 0 (Off) | Checkbox | 紙色を無視して、生成したドットパターンを元の入力画像に直接オーバーレイ合成します。 |
...
3. **Jenkins Hashベースの連続的擬似乱数ノイズ (Continuous PRNG)**
   Jitter機能（位置、サイズ、アスペクト比のランダム変動）を実装する際、`sin` や `fract` などの超越関数を用いた浮動小数点ハッシュはGPUアーキテクチャ（CUDA, Metal, OpenCL）ごとに精度による微小な差異やアーティファクト（バンディング）を引き起こします。本プラグインでは、ビット演算のみによる堅牢な「Jenkins Hash」アルゴリズムを採用し、どの環境でも数学的に完全に一致するノイズパターンをノーコストで生成します。さらに、小数の `Jitter Noise Phase` に対して整数部と次整数部のハッシュを算出し、Hermite補間（`Smoothstep`）でブレンドすることで「アニメーションさせてもカクつかない滑らかで連続的なノイズ」を実現しています。

4. **安全な関数化アーキテクチャ (Struct/Device Function Design)**
   最も複雑な最近傍セルの探索ロジック（`UPDATE_BEST_CELL`）において、DaVinci Resolveの `DVIPComputeNode` (特にCUDA/OpenCL環境) で引き起こされる `typedef struct` の文字列パースエラーやアドレス空間バインディング問題に対処しています。ピュアなC言語スタイルの `struct` を参照渡しする `__DEVICE__` 関数として安全に切り出すことで、MetalとCUDA両環境での高い互換性とレジスタの最適化を両立しています。

5. **数学的対称性を持つ Aspect Jitter**
   Jitter機能によってドットの縦横比（Aspect Ratio）をランダムにブレさせる際、単純な加算や乗算では「縦の伸び」と「横の潰れ」の視覚的バランスが崩れてしまいます。本プラグインでは、UIスライダーと同じ対数スケールカーブ（`_exp2f`）を用いて乱数を指数展開することで、1ピクセルの計算コストの増大を防ぎながら、数学的に完璧なシンメトリーを持つ有機的な歪みを実現しています。
...
7. **セル単位の定数事前計算 (Cell-Specific Pre-calculation)**
   回転行列の計算に必要な三角関数 (`_cosf`, `_sinf`) やアスペクト比の逆数 (`_frecip`) は、セルごとに一意です。これらを描画ピクセルごとに計算するのではなく、`CellInfoKernel` で一度だけ計算してバッファに格納しておくことで、描画フェーズの演算負荷を数千分の一に削減しています。

---

## 8. 実践的な使い方・Tips (Practical Use Cases & Tips)
...
- **将来の拡張**: 現在は RGB シフトや高度なハッチング描画にバッファをフル活用しています。さらなる特殊エフェクトや、より複雑なディザリング・パターンの追加を計画できる設計基盤となっています。

---
*Generated based on source code MugAdvancedHalftone.fuse v3.0*
