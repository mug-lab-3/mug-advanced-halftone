...
> [!NOTE]
> 初期状態では、配置先となる `Fuses` や `Templates/Edit/Effects` などのフォルダ自体が存在しない場合があります。その場合は、手動で新しいフォルダを作成してファイルを配置してください

> [!TIP]
> **各OSでの簡単なアクセス方法**
> - **Windows**: エクスプローラーのアドレスバーに上記のパスをコピペして `Enter` を押すだけで移動できます。
> - **Mac**: `Library` フォルダが隠しフォルダになっているため普通に探すと見つかりません。Finderで `Cmd + Shift + G` を押し、「フォルダの場所を入力」画面にコピペして移動するのが一番簡単です。
...
- **デフォルト値の変更**: ドット色と背景色の初期値を、汎用性の高いニュートラルな色に変更しました。

### v2.34 (2026/04/13)
- **Mac環境への対応**: Macにおいてエフェクトが正常にビルド・表示されない問題を修正しました。
- **内部処理のリファクタリング**: GPUカーネル内のデータ構造とメモリ管理を整理し、より幅広いハードウェア環境での安定性を確保しました。
- **バージョン管理の改善**: 内部バージョン情報の保持形式を最適化し、外部パッケージ管理ツールとの互換性を向上させました。
